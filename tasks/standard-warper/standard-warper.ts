...
task('deploy:standard:standard-warper', 'Deploy the StandardWarper contract')
  ...
  .addParam('allowMultipleRentals', 'Specifies whether its allowed to rent multiple times', undefined, types.boolean, true)
  .addParam('allowConcurrentRentals', 'Specifies whether its allowed to rent multiple rentals at the same time', undefined, types.boolean, true)
  .setAction(async ({ original, metahub, universeRewardAddress, zeroBalanceCheckAddresses, allowMultipleRentals, allowConcurrentRentals }, hre) => {
    const deployer = await hre.ethers.getNamedSigner('deployer');

    //eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
    console.log('Deploying...', { original, metahub, universeRewardAddress, zeroBalanceCheckAddresses, allowMultipleRentals, allowConcurrentRentals });
    ...
    const initData = defaultAbiCoder.encode(
      ...
      [original, metahub, universeRewardAddress, zeroBalanceCheckAddresses, allowMultipleRentals, allowConcurrentRentals],
    );
