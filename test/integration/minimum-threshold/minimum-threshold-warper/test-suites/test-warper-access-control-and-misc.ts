import { Auth__factory, MinimumThresholdWarper, SolidityInterfaces } from '../../../../../typechain';
...
export function testWarperAccessControlAndMisc(): void {
  /**** Contracts ****/
  let minimumTresholdWarper: MinimumThresholdWarper;
  /**** Mocks & Samples ****/
  ...
  beforeEach(async function () {
    /**** Contracts ****/
    minimumTresholdWarper = this.contracts.minimumThreshold.minimumThresholdWarper;
    /**** Mocks & Samples ****/
    ...
    await minimumTresholdWarper.connect(deployer).transferOwnership(warperOwner.address);
    await Auth__factory.connect(minimumTresholdWarper.address, warperOwner).setAuthorizationStatus(
  ...
  it(`does not work when __onRent is called by not Renting Manager`, async () => {
    await expect(
      minimumTresholdWarper.connect(stranger).__onRent(
        0,
        {
          warpedAssets: [],
          universeId: 0,
    ...
    ).to.be.revertedWithCustomError(minimumTresholdWarper, 'CallerIsNotRentingManager');
  });

  it('supports necessary interfaces', async () => {
    await expect(
      minimumTresholdWarper
        .connect(stranger)
    ...
    await expect(
      minimumTresholdWarper
        .connect(stranger)
        .supportsInterface(await getSolidityInterfaceId(solidityInterfaces, 'IMinimumThresholdWarper')),
    ).to.be.fulfilled;
