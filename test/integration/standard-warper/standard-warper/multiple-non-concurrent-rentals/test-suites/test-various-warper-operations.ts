...
export function testVariousWarperOperations(): void {
  ...
  let multipleNonConcurrentRentalsWarper: StandardWarper;
  ...
  beforeEach(async function () {
    ...
    multipleNonConcurrentRentalsWarper = this.contracts.multipleNonCurrentRentals.multipleNonConcurrentRentals;
    /**** Mocks & Samples ****/
    ...
    await multipleNonConcurrentRentalsWarper.connect(deployer).transferOwnership(universeOwner.address);
  ...
  context('Renting `ZERO BALANCE REWARD WARPER ` with various cases', () => {
    ...
    beforeEach(async () => {
      ...
      const setupUniverseTx = await universeWizardV1Adapter.setupUniverseAndRegisterExistingWarper(
        ...
        AddressTranslator.createAssetType(
          new AccountId({ chainId, address: multipleNonConcurrentRentalsWarper.address }),
          'erc721',
        ),
        universeTaxTerms,
        {
      ...
      await Auth__factory.connect(multipleNonConcurrentRentalsWarper.address, universeOwner).setAuthorizationStatus(
    ...
    it(`works with ${LISTING_STRATEGIES.FIXED_RATE_WITH_REWARD} strategy`, async () => {
        ...
        await listingTermsRegistry.allListingTerms(
          {
            ...
            warperAddress: multipleNonConcurrentRentalsWarper.address,
          },
          0,
          1,
        )
      ...
      const rentingEstimationParams = makeSDKRentingEstimationParamsERC721(
        chainId,
        listingId,
        multipleNonConcurrentRentalsWarper.address,
        renterA.address,
        RENTAL_A_PERIOD,
        baseToken.address,
        listingTermsId,
      ...
      await expect(rentTx)
        .to.emit(multipleNonConcurrentRentalsWarper, 'OnRentHookEvent')
        .withArgs(renterA.address, LISTER_TOKEN_ID_1, rentalId);

      await expect(
        multipleNonConcurrentRentalsWarper.connect(stranger).ownerOf(LISTER_TOKEN_ID_1),
      ).to.be.eventually.equal(renterA.address);
      await expect(
        ZeroBalanceWarper__factory.connect(multipleNonConcurrentRentalsWarper.address, stranger).getLastActiveRentalId(
          renterA.address,
          LISTER_TOKEN_ID_1,
        ),
    ...
    it(`reverts when balance is not zero #2`, async () => {
      ...
      await multipleNonConcurrentRentalsWarper
        .connect(universeOwner)
        .setZeroBalanceAddresses([testZeroBalanceCollection.address, multipleNonConcurrentRentalsWarper.address]);
