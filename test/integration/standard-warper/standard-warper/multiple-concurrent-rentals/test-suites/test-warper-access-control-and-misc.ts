...
export function testWarperAccessControlAndMisc(): void {
  /**** Contracts ****/
  let multipleConcurrentRentals: ZeroBalanceWarper;
  /**** Mocks & Samples ****/
  ...
  beforeEach(async function () {
    /**** Contracts ****/
    multipleConcurrentRentals = this.contracts.multipleConcurrentRentals.multipleConcurrentRentals;
    /**** Mocks & Samples ****/
    ...
    await multipleConcurrentRentals.connect(deployer).transferOwnership(warperOwner.address);
    await Auth__factory.connect(multipleConcurrentRentals.address, warperOwner).setAuthorizationStatus(
  ...
  it(`does not work when __onRent is called by not Renting Manager`, async () => {
    await expect(
      multipleConcurrentRentals.connect(stranger).__onRent(
        0,
        {
          warpedAssets: [],
          universeId: 0,
    ...
    ).to.be.revertedWithCustomError(multipleConcurrentRentals, 'CallerIsNotRentingManager');
  });
