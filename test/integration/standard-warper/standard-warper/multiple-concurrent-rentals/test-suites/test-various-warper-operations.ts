...
export function testVariousWarperOperations(): void {
  ...
  let multipleConcurrentRentalsWarper: StandardWarper;
  ...
  beforeEach(async function () {
    ...
    testZeroBalanceCollection = this.contracts.multipleConcurrentRentals.testZeroBalanceCollection;
    multipleConcurrentRentalsWarper = this.contracts.multipleConcurrentRentals.multipleConcurrentRentals;
    /**** Mocks & Samples ****/
    ...
    await multipleConcurrentRentalsWarper.connect(deployer).transferOwnership(universeOwner.address);
  ...
  context('Renting `ZERO BALANCE REWARD WARPER ` with various cases', () => {
    ...
    beforeEach(async () => {
      ...
      const setupUniverseTx = await universeWizardV1Adapter.setupUniverseAndRegisterExistingWarper(
        ...
        AddressTranslator.createAssetType(
          new AccountId({ chainId, address: multipleConcurrentRentalsWarper.address }),
          'erc721',
        ),
        universeTaxTerms,
        {
      ...
      await Auth__factory.connect(multipleConcurrentRentalsWarper.address, universeOwner).setAuthorizationStatus(
    ...
    it(`works with ${LISTING_STRATEGIES.FIXED_RATE_WITH_REWARD} strategy`, async () => {
        ...
        await listingTermsRegistry.allListingTerms(
          {
            ...
            warperAddress: multipleConcurrentRentalsWarper.address,
          },
          0,
          1,
        )
      ...
      const rentingEstimationParams = makeSDKRentingEstimationParamsERC721(
        chainId,
        listingId,
        multipleConcurrentRentalsWarper.address,
        renterA.address,
        RENTAL_A_PERIOD,
        baseToken.address,
        listingTermsId,
      ...
      await expect(rentTx)
        .to.emit(multipleConcurrentRentalsWarper, 'OnRentHookEvent')
        .withArgs(renterA.address, LISTER_TOKEN_ID_1, rentalId);

      await expect(multipleConcurrentRentalsWarper.connect(stranger).ownerOf(LISTER_TOKEN_ID_1)).to.be.eventually.equal(
        renterA.address,
      );
      await expect(
        ZeroBalanceWarper__factory.connect(multipleConcurrentRentalsWarper.address, stranger).getLastActiveRentalId(
          renterA.address,
          LISTER_TOKEN_ID_1,
        ),
      ).to.be.eventually.equal(rentalId);
      await expect(
        ZeroBalanceWarper__factory.connect(
          multipleConcurrentRentalsWarper.address,
          stranger,
        ).getUniverseRewardAddress(),
    ...
    it(`works properly with multi-rental and retrieving reward data`, async () => {
      ...
      await expect(multipleConcurrentRentalsWarper.connect(stranger).ownerOf(LISTER_TOKEN_ID_2)).to.be.eventually.equal(
        renterB.address,
      );
      await expect(
        ZeroBalanceWarper__factory.connect(multipleConcurrentRentalsWarper.address, stranger).getLastActiveRentalId(
    ...
    it(`reverts when balance is not zero`, async () => {
      ...
      await expect(rentingManagerAdapterA.estimateRent(rentingEstimationParams_A))
        .to.be.revertedWithCustomError(multipleConcurrentRentalsWarper, 'AssetIsNotRentable')
    ...
    it(`reverts when balance is not zero #2`, async () => {
      ...
      await multipleConcurrentRentalsWarper
        .connect(universeOwner)
        .setZeroBalanceAddresses([testZeroBalanceCollection.address, multipleConcurrentRentalsWarper.address]);
