...
export function testVariousWarperOperations(): void {
  ...
  let singleNonConcurrentRentalsWarper: StandardWarper;
  ...
  beforeEach(async function () {
    ...
    testZeroBalanceCollection = this.contracts.multipleNonCurrentRentals.testZeroBalanceCollection;
    singleNonConcurrentRentalsWarper = this.contracts.multipleNonCurrentRentals.multipleNonConcurrentRentals;
    /**** Mocks & Samples ****/
    ...
    await singleNonConcurrentRentalsWarper.connect(deployer).transferOwnership(universeOwner.address);
  ...
  context('Renting `ZERO BALANCE REWARD WARPER ` with various cases', () => {
    ...
    beforeEach(async () => {
      ...
      const setupUniverseTx = await universeWizardV1Adapter.setupUniverseAndRegisterExistingWarper(
        ...
        AddressTranslator.createAssetType(
          new AccountId({ chainId, address: singleNonConcurrentRentalsWarper.address }),
          'erc721',
        ),
        universeTaxTerms,
        {
      ...
      await Auth__factory.connect(singleNonConcurrentRentalsWarper.address, universeOwner).setAuthorizationStatus(
    ...
    it(`works with ${LISTING_STRATEGIES.FIXED_RATE_WITH_REWARD} strategy`, async () => {
        ...
        await listingTermsRegistry.allListingTerms(
          {
            ...
            warperAddress: singleNonConcurrentRentalsWarper.address,
          },
          0,
          1,
        )
      ...
      const rentingEstimationParams = makeSDKRentingEstimationParamsERC721(
        chainId,
        listingId,
        singleNonConcurrentRentalsWarper.address,
        renterA.address,
        RENTAL_A_PERIOD,
        baseToken.address,
        listingTermsId,
      ...
      await expect(rentTx)
        .to.emit(singleNonConcurrentRentalsWarper, 'OnRentHookEvent')
        .withArgs(renterA.address, LISTER_TOKEN_ID_1, rentalId);

      await expect(
        singleNonConcurrentRentalsWarper.connect(stranger).ownerOf(LISTER_TOKEN_ID_1),
      ).to.be.eventually.equal(renterA.address);
      await expect(
        ZeroBalanceWarper__factory.connect(singleNonConcurrentRentalsWarper.address, stranger).getLastActiveRentalId(
          renterA.address,
          LISTER_TOKEN_ID_1,
        ),
    ...
    it(`reverts when multi-rental and retrieving reward data`, async () => {
      const LISTING_1_MAX_LOCK_PERIOD = SECONDS_IN_DAY;
      ...
      await expect(rentingManagerAdapterA.estimateRent(rentingEstimationParams_B))
        .to.be.revertedWithCustomError(singleNonConcurrentRentalsWarper, 'AssetIsNotRentable')
        .withArgs('Concurrent rentals are not allowed');
    });
    ...
    it(`reverts when balance is not zero #2`, async () => {
      ...
      await singleNonConcurrentRentalsWarper
        .connect(universeOwner)
        .setZeroBalanceAddresses([testZeroBalanceCollection.address, singleNonConcurrentRentalsWarper.address]);
